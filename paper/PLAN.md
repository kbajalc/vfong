...
Status: done. Abstract, Discussion (§5), Conclusion and future work (§7), and the merged
reference list (DRAFT groups plus HONG-2016) are written; editorial notes removed, STYLE
em-dash check clean. Added §6 "AI as a research collaborator", a disclosure of the human-AI
process (grounded corpus, phased review, division of labour, safeguards against fabrication,
authorship). Reviewer (REVIEW:) comments folded in; Methods and Results split into matching
subsections (3.5-3.9 / 4.2-4.5). The COMP5/COMP55 IROC figures were verified against the
source texts (all correct; one HILB mis-citation and one epidemiology wording fixed), and
"IROC" is glossed as the AUC on first use. All ten figures are exported to `paper/figures/`
and embedded in the manuscript with numbered captions (Figures 1-4 ECG strips in §1.1;
5-10 generated by `PAPER.ipynb`). `PAPER.ipynb` was resynced as a companion: its markdown
mirrors the paper sections and drops the internal "Phase" language.
