...
def filtering(signal,Fs,step2='movingAverage',step2Param=5,highPassFc=1,lowPassFc=30,butterworthOrder=12):
	...
	b, a = butter(1, highPassFc/ Fnyq , btype='highpass')		# definition of the high pass filter # type: ignore
	signal3 = lfilter(b, a, signal2) # type: ignore

	# 4 ) Low pass filter for high frequency suppression

	b,a = butter(12,lowPassFc/Fnyq)								# definition of the high pass filter # type: ignore
...
def processSignal(db,signal,Fs,plot,label,file,episode,channel=1,Te=5):
	...
	IMF = EMD().emd(window2) # type: ignore
