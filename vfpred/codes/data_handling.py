...
def processMITMVADBFile(path,fileNo,Te=5):
	...
	for i in range(len(annotSymbol)): # type: ignore

		annotSymbol[i] = annotSymbol[i].rstrip('\x00') # because the file contains \x00  # type: ignore

		if(annotSymbol[i]=='(N'):		# N = NSR  # type: ignore
			annotSymbol[i]='(NSR'                  # type: ignore

		elif (annotSymbol[i] == '(VFIB'):	# VFIB = VF  # type: ignore
			annotSymbol[i] = '(VF'                       # type: ignore
