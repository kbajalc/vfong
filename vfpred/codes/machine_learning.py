...
def upsampleSMOTE(loadFile='dataSetType3.p',saveFile='smoteData.p'):
	...
	sm = SMOTE()						# smote object  # type: ignore

	Xup , Yup = sm.fit_resample(X,Y)	# generate synthetic data  # type: ignore
	...
	for i in range(len(Xup)):
		...
		if(Yup[i]==1):						# VF feature # type: ignore
...
def svmParameterTuning(file='dataSetType3.p',vfCnt=3000,notvfCnt=5000):
	...
	for gamma in gammas:
		for C in Cs:
			...
			print('"'+str(gamma)+' , '+str(C)+' : ( '+str(pp[0])+' , '+str(pp[1])+' , '+str(pp[2])+'"') # type: ignore
